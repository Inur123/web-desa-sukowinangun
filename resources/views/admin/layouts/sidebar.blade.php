<div class="sidebar bg-white w-64 shadow-lg flex flex-col z-50">
    ...
    <nav class="flex-1 p-4 overflow-y-auto">
        <ul class="space-y-2">
            ...
            <li class="dropdown-container">
                ...
                {{ request()->routeIs('layanan.*') ? 'text-primary bg-primary/10' : 'text-gray-600 hover:text-primary hover:bg-gray-50 transition-colors' }}">
                    <div class="flex items-center">
                ...
                <ul class="ml-4 mt-1 space-y-1 dropdown-menu {{ request()->routeIs('layanan.*') ? '' : 'hidden' }}">
                    ...
                    <li>
                        <a href=""
                            class="flex items-center px-4 py-2 rounded-lg text-sm font-medium
                        {{ request()->routeIs('layanan.sktm') ? 'text-primary bg-primary/10' : 'text-gray-600 hover:text-primary hover:bg-gray-50 transition-colors' }}">
    ...
    <div class="p-4 border-t border-gray-200 mt-auto">
        <div class="flex items-center">
            <div class="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm uppercase">
        ...
        <form method="POST" action="{{ route('logout') }}" class="w-full">
            @csrf
            <button type="submit" class="w-full mt-3 px-4 py-2 text-sm text-gray-600 hover:text-red-600 transition-colors">
