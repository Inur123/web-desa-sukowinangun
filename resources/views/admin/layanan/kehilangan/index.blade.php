    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-lg font-semibold text-gray-800 mb-2 sm:mb-0">Daftar Pengajuan Surat Kehilangan</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dokumen Hilang</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($kehilangans as $kehilangan)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $loop->iteration }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $kehilangan->nama }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($kehilangan->ttl)->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $kehilangan->alamat }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $kehilangan->surat_yang_hilang }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $kehilangan->keperluan }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($kehilangan->created_at)->format('H:i | d M Y ') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('kehilangan.show', $kehilangan->id) }}"
                                    class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-eye"></i></a>
                                <form action="{{ route('kehilangan.destroy', $kehilangan->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-600">
                    Menampilkan {{ $kehilangans->firstItem() }}-{{ $kehilangans->lastItem() }} dari {{ $kehilangans->total() }} Pengajuan Kehilangan
                </div>
                <div class="flex items-center space-x-2">
                    @if ($kehilangans->onFirstPage())
                        <span class="px-2 py-1 text-sm text-gray-400 cursor-not-allowed">
                            <i class="fas fa-chevron-left"></i>
                        </span>
                    @else
                        <a href="{{ $kehilangans->previousPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
                            <i class="fas fa-chevron-left"></i>
                        </a>
                    @endif

                    @for ($i = 1; $i <= $kehilangans->lastPage(); $i++)
                        @if ($i == $kehilangans->currentPage())
                            <span class="px-2 py-1 text-sm bg-primary text-white rounded">{{ $i }}</span>
                        @else
                            <a href="{{ $kehilangans->url($i) }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">{{ $i }}</a>
                        @endif
                    @endfor

                    @if ($kehilangans->hasMorePages())
                        <a href="{{ $kehilangans->nextPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
