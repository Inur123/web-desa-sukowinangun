...
<div class="bg-white rounded-lg shadow overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-lg font-semibold text-gray-800 mb-2 sm:mb-0">Daftar Pengajuan Harga Tanah</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Alamat Tanah</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Harga/m²</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($hargaTanahs as $hargaTanah)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $loop->iteration }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $hargaTanah->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $hargaTanah->alamat_tanah }}</td>
                   <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $hargaTanah->harga_per_meter }}</td>
</td>

                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ Str::limit($hargaTanah->keperluan, 20) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                            {{ \Carbon\Carbon::parse($hargaTanah->created_at)->format('H:i | d M Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('harga-tanah.show', $hargaTanah->id) }}"
                                class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-eye"></i></a>
                            <form action="{{ route('harga-tanah.destroy', $hargaTanah->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-600">
                    Menampilkan {{ $hargaTanahs->firstItem() }}-{{ $hargaTanahs->lastItem() }} dari {{ $hargaTanahs->total() }} Pengajuan
                </div>
                <div class="flex items-center space-x-2">
                    @if ($hargaTanahs->onFirstPage())
                        <span class="px-2 py-1 text-sm text-gray-400 cursor-not-allowed">
                            <i class="fas fa-chevron-left"></i>
                        </span>
                    @else
                        <a href="{{ $hargaTanahs->previousPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
                            <i class="fas fa-chevron-left"></i>
                        </a>
                    @endif

                    @for ($i = 1; $i <= $hargaTanahs->lastPage(); $i++)
                        @if ($i == $hargaTanahs->currentPage())
                            <span class="px-2 py-1 text-sm bg-primary text-white rounded">{{ $i }}</span>
                        @else
                            <a href="{{ $hargaTanahs->url($i) }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">{{ $i }}</a>
                        @endif
                    @endfor

                    @if ($hargaTanahs->hasMorePages())
                        <a href="{{ $hargaTanahs->nextPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
