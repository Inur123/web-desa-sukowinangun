    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-lg font-semibold text-gray-800 mb-2 sm:mb-0">Daftar Pengajuan Surat Belum Menikah</h2>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($belumMenikahs as $belumMenikah)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $loop->iteration }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $belumMenikah->nama }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($belumMenikah->ttl)->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $belumMenikah->alamat }}</td>

                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $belumMenikah->keperluan }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($belumMenikah->created_at)->format('H:i | d M Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('belum-menikah.show', $belumMenikah->id) }}"
                                    class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-eye"></i></a>
                                <form action="{{ route('belum-menikah.destroy', $belumMenikah->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Menampilkan {{ $belumMenikahs->firstItem() }}-{{ $belumMenikahs->lastItem() }} dari {{ $belumMenikahs->total() }} Pengajuan Belum Menikah
                    </div>
                    <div class="flex items-center space-x-2">
                        @if ($belumMenikahs->onFirstPage())
                            <span class="px-2 py-1 text-sm text-gray-400 cursor-not-allowed">
                                <i class="fas fa-chevron-left"></i>
                            </span>
                        @else
                            <a href="{{ $belumMenikahs->previousPageUrl() }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
                                <i class="fas fa-chevron-left"></i>
                            </a>
                        @endif

                        @for ($i = 1; $i <= $belumMenikahs->lastPage(); $i++)
                            @if ($i == $belumMenikahs->currentPage())
                                <span class="px-2 py-1 text-sm bg-primary text-white rounded">{{ $i }}</span>
                            @else
                                <a href="{{ $belumMenikahs->url($i) }}"
                                    class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">{{ $i }}</a>
                            @endif
                        @endfor

                        @if ($belumMenikahs->hasMorePages())
                            <a href="{{ $belumMenikahs->nextPageUrl() }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
