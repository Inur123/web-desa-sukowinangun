...
<div class="bg-white rounded-lg shadow overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-lg font-semibold text-gray-800 mb-2 sm:mb-0">Daftar Pengajuan Surat Penghasilan</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Penghasilan/Bulan</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($penghasilans as $penghasilan)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $loop->iteration }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $penghasilan->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $penghasilan->penghasilan_per_bulan }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ Str::limit($penghasilan->keperluan, 20) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                            {{ \Carbon\Carbon::parse($penghasilan->created_at)->format('H:i | d M Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('penghasilan.show', $penghasilan->id) }}"
                                class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-eye"></i></a>
                            <form action="{{ route('penghasilan.destroy', $penghasilan->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada data pengajuan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="px-6 py-4 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-600">
                    Menampilkan {{ $penghasilans->firstItem() }}-{{ $penghasilans->lastItem() }} dari {{ $penghasilans->total() }} Pengajuan
                </div>
                <div class="flex items-center space-x-2">
                    @if ($penghasilans->onFirstPage())
                        <span class="px-2 py-1 text-sm text-gray-400 cursor-not-allowed">
                            <i class="fas fa-chevron-left"></i>
                        </span>
                    @else
                        <a href="{{ $penghasilans->previousPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
                            <i class="fas fa-chevron-left"></i>
                        </a>
                    @endif

                    @for ($i = 1; $i <= $penghasilans->lastPage(); $i++)
                        @if ($i == $penghasilans->currentPage())
                            <span class="px-2 py-1 text-sm bg-primary text-white rounded">{{ $i }}</span>
                        @else
                            <a href="{{ $penghasilans->url($i) }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">{{ $i }}</a>
                        @endif
                    @endfor

                    @if ($penghasilans->hasMorePages())
                        <a href="{{ $penghasilans->nextPageUrl() }}"
                            class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
