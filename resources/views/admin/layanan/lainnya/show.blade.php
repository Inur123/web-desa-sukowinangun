...
@section('title', 'Detail Layanan Lainnya - ' . $lainnya->nama)
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-800">Detail Pengajuan Layanan Lainnya</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->nama }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Tempat/Tanggal Lahir</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->tempat_lahir }}, {{ \Carbon\Carbon::parse($lainnya->ttl)->format('d F Y') }}
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">NIK</label>
                        <div class="mt-1 flex items-center">
                            <p id="nik-text" class="text-sm text-gray-900">{{ substr($lainnya->nik, 0, 5) }}<span class="blur-sm">{{ substr($lainnya->nik, 5) }}</span>
                            </p>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->alamat }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Status Perkawinan</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->status_perkawinan }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">No. HP</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->no_hp }}</p>
                    </div>
                </div>

                <!-- Data Layanan -->
                <div class="space-y-4">
                    <h3 class="text-lg font-medium text-gray-900 border-b pb-2">Data Layanan</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Keperluan</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $lainnya->keperluan }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    @foreach($lainnya->files as $file)
                        <div class="border rounded-lg p-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">{{ $file->nama }}</label>
                            <div class="border rounded-lg p-4">
                                <div class="flex items-center justify-between">
                                  <a href="{{ route('lainnya.showFile', ['id' => $lainnya->id, 'fileId' => $file->id]) }}" target="_blank">
    <i class="fas fa-file-pdf mr-2"></i> Lihat Berkas
                                    ...
                                    @if($isImage)
                                        <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                           <img src="{{ route('lainnya.showFile', ['id' => $lainnya->id, 'fileId' => $file->id]) }}"
     alt="Preview Berkas" class="w-full h-full object-contain">
            ...
            @if ($lainnya->status == 'baru')
                <div class="mt-8 pt-4 border-t border-gray-200 flex justify-end space-x-3">
                    <form action="{{ route('lainnya.approve', $lainnya->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                    ...
                    <form action="{{ route('lainnya.reject', $lainnya->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
