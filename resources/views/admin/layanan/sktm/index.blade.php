    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-lg font-semibold text-gray-800 mb-2 sm:mb-0">Daftar Pengajuan Surat Keterangan Tidak Mampu</h2>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($sktms as $sktm)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $sktm->nama }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($sktm->ttl)->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $sktm->alamat }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $sktm->nama_anak }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $sktm->keperluan }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                {{ \Carbon\Carbon::parse($sktm->created_at)->format('H:i | d M Y ') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('sktm.show', $sktm->id) }}"
                                    class="text-blue-600 hover:text-blue-900 mr-3"><i class="fas fa-eye"></i></a>

                                <form action="{{ route('sktm.destroy', $sktm->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Menampilkan {{ $sktms->firstItem() }}-{{ $sktms->lastItem() }} dari {{ $sktms->total() }} SKTM
                    </div>
                    <div class="flex items-center space-x-2">
                        @if ($sktms->onFirstPage())
                            <span class="px-2 py-1 text-sm text-gray-400 cursor-not-allowed">
                                <i class="fas fa-chevron-left"></i>
                            </span>
                        @else
                            <a href="{{ $sktms->previousPageUrl() }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
                                <i class="fas fa-chevron-left"></i>
                            </a>
                        @endif

                        @for ($i = 1; $i <= $sktms->lastPage(); $i++)
                            @if ($i == $sktms->currentPage())
                        ...
                        @if ($sktms->hasMorePages())
                            <a href="{{ $sktms->nextPageUrl() }}"
                                class="px-2 py-1 text-sm text-gray-500 hover:text-gray-700 transition-colors">
