...
@section('title', 'Detail Pengantar SKCK - ' . $pengantarSkck->nama)
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-800">Detail Pengajuan Pengantar SKCK</h2>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $pengantarSkck->nama }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Tempat/Tanggal Lahir</label>
                        <p class="mt-1 text-sm text-gray-900">
                            {{ $pengantarSkck->tempat_lahir }}, {{ \Carbon\Carbon::parse($pengantarSkck->ttl)->format('d F Y') }}
                        </p>
                    </div>
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                            <p id="nik-text" class="text-sm text-gray-900">
                                {{ substr($pengantarSkck->nik, 0, 5) }}<span class="blur-sm">{{ substr($pengantarSkck->nik, 5) }}</span>
                            </p>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $pengantarSkck->alamat }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Status Perkawinan</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $pengantarSkck->status_perkawinan }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">No. HP</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $pengantarSkck->no_hp }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Keperluan</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $pengantarSkck->keperluan }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="border rounded-lg p-4">
                            ...
                            @if ($pengantarSkck->ktp)
                                @php
                                    $ktpExtension = pathinfo($pengantarSkck->ktp, PATHINFO_EXTENSION);
                                @endphp
                                <div class="border rounded-lg p-4">
                                    <div class="flex items-center justify-between">
                                        <a href="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'ktp']) }}"
                                        ...
                                        @if (in_array(strtolower($ktpExtension), ['jpg', 'jpeg', 'png']))
                                            <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                                <img src="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'ktp']) }}"
                        ...
                        <div class="border rounded-lg p-4">
                            ...
                            @if ($pengantarSkck->kk)
                                @php
                                    $kkExtension = pathinfo($pengantarSkck->kk, PATHINFO_EXTENSION);
                                @endphp
                                <div class="border rounded-lg p-4">
                                    <div class="flex items-center justify-between">
                                        <a href="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'kk']) }}"
                                            target="_blank" class="text-blue-600 hover:text-blue-800 text-sm flex items-center">
                                            <i class="fas fa-file-pdf mr-2"></i> Lihat Kartu Keluarga
                                        </a>
                                        @if (in_array(strtolower($kkExtension), ['jpg', 'jpeg', 'png']))
                                            <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                                <img src="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'kk']) }}"
                        ...
                        <div class="border rounded-lg p-4">
                            ...
                            @if ($pengantarSkck->pengantar_rt)
                                @php
                                    $pengantarExtension = pathinfo($pengantarSkck->pengantar_rt, PATHINFO_EXTENSION);
                                @endphp
                                <div class="border rounded-lg p-4">
                                    <div class="flex items-center justify-between">
                                        <a href="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'pengantar']) }}"
                                            target="_blank" class="text-blue-600 hover:text-blue-800 text-sm flex items-center">
                                            <i class="fas fa-file-pdf mr-2"></i> Lihat Surat Pengantar
                                        </a>
                                        @if (in_array(strtolower($pengantarExtension), ['jpg', 'jpeg', 'png']))
                                            <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                                <img src="{{ route('pengantar-skck.showFile', ['id' => $pengantarSkck->id, 'type' => 'pengantar']) }}"
            ...
            @if ($pengantarSkck->status == 'baru')
                <div class="mt-8 pt-4 border-t border-gray-200 flex justify-end space-x-3">
                    <form action="{{ route('pengantar-skck.approve', $pengantarSkck->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                    ...
                    <form action="{{ route('pengantar-skck.reject', $pengantarSkck->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
