...
@section('title', 'Detail Surat Kelahiran - ' . $kelahiran->nama)
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-800">Detail Pengajuan Surat Kelahiran</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $kelahiran->nama }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Tempat/Tanggal Lahir</label>
                        <p class="mt-1 text-sm text-gray-900">
                            {{ $kelahiran->tempat_lahir }}, {{ \Carbon\Carbon::parse($kelahiran->ttl)->format('d F Y') }}
                        </p>
                    </div>
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                            <p id="nik-text" class="text-sm text-gray-900">
                                {{ substr($kelahiran->nik, 0, 5) }}<span class="blur-sm">{{ substr($kelahiran->nik, 5) }}</span>
                            </p>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $kelahiran->alamat }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Status Perkawinan</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $kelahiran->status_perkawinan }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">No. HP</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $kelahiran->no_hp }}</p>
                    </div>
                </div>

                <!-- Data Kelahiran -->
                <div class="space-y-4">
                    <h3 class="text-lg font-medium text-gray-900 border-b pb-2">Data Kelahiran</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Tanggal Pengajuan</label>
                        <p class="mt-1 text-sm text-gray-900">
                            {{ \Carbon\Carbon::parse($kelahiran->created_at)->format('d F Y H:i') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        ...
        <div class="space-y-4">
            ...
            <div class="border rounded-lg p-4">
                ...
                @if ($kelahiran->ktp)
                    @php
                        $ktpExtension = pathinfo($kelahiran->getRawOriginal('ktp'), PATHINFO_EXTENSION);
                    @endphp
                    <div class="border rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <a href="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'ktp']) }}"
                            ...
                            @if (in_array(strtolower($ktpExtension), ['jpg', 'jpeg', 'png']))
                                <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                    <img src="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'ktp']) }}"
            ...
            <div class="border rounded-lg p-4">
                ...
                @if ($kelahiran->kk)
                    @php
                        $kkExtension = pathinfo($kelahiran->getRawOriginal('kk'), PATHINFO_EXTENSION);
                    @endphp
                    <div class="border rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <a href="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'kk']) }}"
                                target="_blank" class="text-blue-600 hover:text-blue-800 text-sm flex items-center">
                                <i class="fas fa-file-pdf mr-2"></i> Lihat Kartu Keluarga
                            </a>
                            @if (in_array(strtolower($kkExtension), ['jpg', 'jpeg', 'png']))
                                <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                    <img src="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'kk']) }}"
        ...
        <div class="space-y-4">
            ...
            <div class="border rounded-lg p-4">
                ...
                @if ($kelahiran->pengantar_rt)
                    @php
                        $pengantarExtension = pathinfo($kelahiran->getRawOriginal('pengantar_rt'), PATHINFO_EXTENSION);
                    @endphp
                    <div class="border rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <a href="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'pengantar']) }}"
                                target="_blank" class="text-blue-600 hover:text-blue-800 text-sm flex items-center">
                                <i class="fas fa-file-pdf mr-2"></i> Lihat Surat Pengantar
                            </a>
                            @if (in_array(strtolower($pengantarExtension), ['jpg', 'jpeg', 'png']))
                                <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                    <img src="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'pengantar']) }}"
            ...
            <!-- Surat Keterangan Bidan/Dokter -->
            <div class="border rounded-lg p-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Surat Keterangan Bidan/Dokter</label>
                @if ($kelahiran->surat_keterangan_bidan)
                    @php
                        $bidanExtension = pathinfo($kelahiran->getRawOriginal('surat_keterangan_bidan'), PATHINFO_EXTENSION);
                    @endphp
                    <div class="border rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <a href="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'bidan']) }}"
                                target="_blank" class="text-blue-600 hover:text-blue-800 text-sm flex items-center">
                                <i class="fas fa-file-pdf mr-2"></i> Lihat Surat Bidan/Dokter
                            </a>
                            @if (in_array(strtolower($bidanExtension), ['jpg', 'jpeg', 'png']))
                                <div class="w-32 h-20 bg-gray-100 rounded overflow-hidden ml-4">
                                    <img src="{{ route('kelahiran.showFile', ['id' => $kelahiran->id, 'type' => 'bidan']) }}"
                                        alt="Preview Surat Bidan/Dokter" class="w-full h-full object-contain">
                                </div>
                            @else
                                <span class="text-sm text-gray-500 ml-4">Preview tidak tersedia</span>
                            @endif
            ...
            @if ($kelahiran->status == 'baru')
                <div class="mt-8 pt-4 border-t border-gray-200 flex justify-end space-x-3">
                    <form action="{{ route('kelahiran.approve', $kelahiran->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                    ...
                    <form action="{{ route('kelahiran.reject', $kelahiran->id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
