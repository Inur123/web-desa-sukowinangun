...
@section('title', 'Layanan Surat Kelahiran - Sukowinangun')
    ...
    <section class="pt-16 bg-gradient-to-r from-primary to-secondary">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
            <div class="text-center text-white">
                ...
                <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4">Form Pengajuan Surat Kelahiran</h1>
                <p class="text-base sm:text-lg md:text-xl text-gray-100 max-w-3xl mx-auto px-2 sm:px-0">
                    Ajukan permohonan Surat Keterangan Kelahiran untuk keperluan administrasi kependudukan.
                </p>
            </div>
        </div>
    </section>
                        ...
                        <form action="{{ route('kelahiran.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="border-t pt-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Surat Keterangan
                                            Bidan/Dokter</label>
                                        <div class="space-y-4">
                                            <!-- File Upload Option -->
                                            <div id="bidan_file_container"
                                                class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-secondary transition-colors cursor-pointer">
                                                <div id="bidan_file_placeholder">
                                                    <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 mb-2"></i>
                                                    <p class="text-gray-600">Upload Surat Keterangan Bidan/Dokter</p>
                                                    <p class="text-xs text-gray-500 mt-1">PDF, JPG, PNG (Max 2MB)</p>
                                                </div>
                                                <div id="bidan_file_preview" class="hidden">
                                                    <div class="flex items-center justify-between bg-gray-50 p-2 rounded">
                                                        <div class="flex items-center truncate">
                                                            <i class="fas fa-file-pdf text-red-500 text-xl mr-2"></i>
                                                            <span id="bidan_file_name" class="truncate"></span>
                                                        </div>
                                                        <button type="button" onclick="resetFileInput('bidan_file')"
                                                            class="text-red-500 hover:text-red-700">
                                                            <i class="fas fa-times"></i>
                                                        </button>
                                                    </div>
                                                </div>
                                                <input type="file" id="surat_keterangan_bidan"
                                                    name="surat_keterangan_bidan_file" accept=".pdf,.jpg,.jpeg,.png"
                                                    class="hidden"
                                                    onchange="previewFile('surat_keterangan_bidan', 'bidan_file')">
                                            </div>
                                            ...
                                            <div>
                                                <button type="button"
                                                    onclick="openCamera('bidan_camera', 'Surat Bidan/Dokter')"
                                                    class="w-full bg-gray-100 hover:bg-gray-200 text-gray-800 py-2 px-4 rounded-lg flex items-center justify-center">
                                                    <i class="fas fa-camera mr-2"></i>
                                                    Ambil Foto Surat
                                                </button>
                                                <input type="hidden" id="bidan_camera"
                                                    name="surat_keterangan_bidan_camera">
                                                <div id="bidan_camera_preview" class="mt-2 hidden">
                                                    <img id="bidan_camera_img"
                                                        class="max-w-full h-auto rounded-lg border border-gray-200 max-h-40">
                                                    <button type="button" onclick="resetCameraInput('bidan_camera')"
                                                        class="mt-2 text-red-500 hover:text-red-700 text-sm">
                                                        <i class="fas fa-times mr-1"></i> Hapus Foto
                                                    </button>
                                                </div>
                            ...
                            <div class="border-t pt-6">
                                <button type="submit"
                                    ...
                                    Ajukan Permohonan Surat Kelahiran
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
                ...
                if (e.target.files && e.target.files.length > 0) {
                    ...
                    reader.onload = function(event) {
                        ...
                        compressImageForIOS(event.target.result, function(compressedImage) {
                            ...
                            } else if (inputId === 'bidan_camera') {
                                resetFileInput('bidan_file');
                            }
                        });
                    };
        ...
        function previewFile(inputId, previewId) {
            ...
            if (file) {
                ...
                } else if (previewId === 'bidan_file') {
                    resetCameraInput('bidan_camera');
                }
            }
        }
        ...
        function resetFileInput(type) {
            const inputId = type === 'pengantar_file' ? 'pengantar_rt_file' :
                (type === 'ktp_file' ? 'ktp_file' :
                    (type === 'kk_file' ? 'kk_file' : 'surat_keterangan_bidan'));
            const input = document.getElementById(inputId);
        ...
        // Untuk Surat Bidan
        document.getElementById('bidan_file_container').addEventListener('click', function(e) {
            if (
                e.target.closest('button') ||
                e.target.closest('i') ||
                e.target.closest('svg')
            ...
            document.getElementById('surat_keterangan_bidan').click();
        });
    </script>
@endsection
