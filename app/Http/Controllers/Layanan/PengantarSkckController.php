...
class PengantarSkckController extends Controller
{
    ...
    {
        $pengantarSkcks = PengantarSkck::latest()->paginate(10);
        $total = PengantarSkck::count();
        $disetujui = PengantarSkck::where('status', 'diterima')->count();
        $ditolak = PengantarSkck::where('status', 'ditolak')->count();
        $baru = PengantarSkck::where('status', 'baru')->count();

        return view('admin.layanan.skck.index', compact('pengantarSkcks', 'total', 'disetujui', 'ditolak', 'baru'));
    }
    ...
    public function store(Request $request)
    {
        try {
           ...
           if (!$request->hasFile('pengantar_rt_file') && !$request->filled('pengantar_rt_camera')) {
            ...
            // Buat objek baru
            $pengantarSkck = new PengantarSkck($validated);

            // Atur file/kamera
            $pengantarSkck->pengantar_rt = $request->file('pengantar_rt_file') ?? $request->input('pengantar_rt_camera');
            $pengantarSkck->ktp = $request->file('ktp_file') ?? $request->input('ktp_camera');
            $pengantarSkck->kk = $request->file('kk_file') ?? $request->input('kk_camera');

            // Simpan
            $pengantarSkck->save();

            // Kirim notifikasi ke user
            $userMessage = "Halo {$validated['nama']},\n\nTerima kasih telah mengajukan Pengantar SKCK. Pengajuan Anda telah kami terima dengan detail sebagai berikut:\n\nKeperluan: {$validated['keperluan']}\n\nKami akan memproses pengajuan Anda segera. Anda akan mendapatkan notifikasi berikutnya ketika status pengajuan berubah.\n\nSalam hormat,\nAdmin";
            ...
            return redirect()->back()
                ->with('success', 'Pengajuan Pengantar SKCK berhasil dikirim.');
        } catch (\Exception $e) {
            return redirect()->back()
                ->withInput()
    ...
    {
        $pengantarSkck = PengantarSkck::findOrFail($id);
        return view('admin.layanan.skck.show', compact('pengantarSkck'));
    }
    ...
    public function showFile($id, $type)
    {
        ...
        // Get file path
        $path = $pengantarSkck->getRawOriginal($type === 'ktp' ? 'ktp' : ($type === 'pengantar' ? 'pengantar_rt' : 'kk'));
        ...
        try {
            ...
            $displayName = ($type === 'ktp' ? 'KTP_' : ($type === 'pengantar' ? 'Pengantar_RT_' : 'KK_')) . $pengantarSkck->nama . '.' . $extension;
    ...
    public function destroy($id)
    {
        try {
            ...
            // Hapus file jika ada
            $ktpPath = $pengantarSkck->getRawOriginal('ktp');
            $pengantarPath = $pengantarSkck->getRawOriginal('pengantar_rt');
            $kkPath = $pengantarSkck->getRawOriginal('kk');
